...
with anna as (
  ...
  select id, 'client_created', 'Клиент добавлен', null, '2026-07-06T10:00:00Z'::timestamptz from anna
  union all
  select id, 'client_created', 'Клиент добавлен', null, '2026-07-04T14:30:00Z' from mikhail
  union all
  select id, 'note_added', 'Проведена первичная консультация, договор отправлен на проверку', null, '2026-07-05T10:00:00Z' from mikhail
  union all
  select id, 'status_changed', 'Новый → В работе', '{"from":"new","to":"in_progress"}'::jsonb, '2026-07-05T10:05:00Z' from mikhail
  union all
  select id, 'client_created', 'Клиент добавлен', null, '2026-07-01T11:00:00Z' from alfa
  union all
  ...
  union all
  select id, 'client_created', 'Клиент добавлен', null, '2026-06-20T09:00:00Z' from sergey
  union all
  ...
  union all
  select id, 'Запросить документы повторно', '2026-07-08', '2026-07-05T16:45:00Z' from alfa
  returning id
)
