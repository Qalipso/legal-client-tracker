...
3. Задеплоить `notify-telegram` и `telegram-webhook` (см. §2.2), затем
   ...
   ```
4. В приложении: ⚙ → Настройки → Получатели → **📎 Подключить Telegram** —
   откроется бот, нажать Start, бот сам подтвердит подключение (chat_id
   вводить не нужно, v0.8). Если бот не отвечает на deep link — fallback:
   написать боту `/start` вручную, он ответит приветствием с chat ID,
   готовым для ручного ввода (бот не может писать первым — ограничение
   Telegram, поэтому начинать должны вы).
5. **Отправить тест** → должно прийти «✅ Тестовое уведомление».
