...
## Входящий webhook: `/start` и получение chat_id

Раньше единственный способ узнать свой chat_id — сторонний бот
`@userinfobot`. Функция `telegram-webhook`
(`supabase/functions/telegram-webhook/index.ts`) делает бота Элли
самодостаточным: он отвечает на любое входящее сообщение (в первую очередь
`/start`) приветствием с готовым к копированию chat_id.
