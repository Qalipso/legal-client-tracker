...
**Убрана техническая избыточность в UI, дружелюбнее онбординг**

- **«Данные» в Настройках**: убран список сырых внутренних имён колонок
  («колонки: name, phone, email, telegram, status, note, matterTitle,
  matterType, matterSubject, stage, counterparty, keyDeadline, priority,
  responsibleLawyer)») — юристу не нужно видеть camelCase-имена полей;
  осталась простая фраза «Экспорт всех клиентов и дел в таблицу (CSV)».
- **Ошибки доставки**: `friendlyError()` больше не называет вендора
  («Resend») и не показывает сырые HTTP-коды/ответы API конечному
  пользователю — только действие, которое можно предпринять, или «нет,
  временно недоступно, обратитесь к администратору». Тот же фильтр
  применён к результату «Отправить тест» (раньше показывал сырую строку
  ошибки от Resend/Telegram API).
- **Найден и исправлен баг**, внесённый вместе с `friendlyError()`: она
  применялась ко ВСЕМ строкам истории уведомлений, включая `skipped` —
  это подменяло осмысленные причины пропуска («событие «X» выключено в
  настройках», «нет активных получателей») на общую фразу «не удалось
  отправить». Теперь `friendlyError` применяется только к `status:
  error`; `skipped`-причины показываются как есть (они уже человекочитаемы).
- **Статусы истории на русском**: бейджи показывали сырые английские
  слова `sent`/`error`/`skipped` — теперь «отправлено»/«ошибка»/«пропущено».
- **Онбординг дружелюбнее**: тёплое приветствие вместо сухого заголовка,
  прогресс-бар вместо счётчика «3/4», у каждого невыполненного шага —
  короткое объяснение «зачем» на языке пользователя (не разработчика),
  единственная CTA-кнопка адаптируется под следующий реальный шаг вместо
  того, чтобы всегда вести в Настройки.
