...
- **Email-канал уведомлений** (scaffold) — переключатель + получатели
  готовы, `notify-telegram` умеет отправлять через Resend. **[Not verified]**:
  реальной доставки нет — нет аккаунта/API-ключа Resend
...
- **Реальная доставка email-уведомлений** — нужен Resend/Postmark
  аккаунт + `RESEND_API_KEY`/`RESEND_FROM_EMAIL` в секретах Supabase
...
- Реальная доставка email-уведомлений (Resend/Postmark) — см. Next steps
